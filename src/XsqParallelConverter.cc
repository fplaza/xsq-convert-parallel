...
void XsqParallelConverter::convert(const std::string& input_file, const std::string& output_dir, unsigned nb_processes, const boost::optional<std::vector<std::string> >& prefixes_wanted)
{
	...
	const std::vector<std::string>& libraries = prefixes_wanted ? 
	...
	const CommandLines& command_lines = 
...
XsqParallelConverter::CommandLines XsqParallelConverter::build_command_lines(const std::vector<std::string>& libraries, const std::string& input_file, const std::string& output_dir, unsigned nb_processes)
{
	...
	CommandLines command_lines(nb_processes, boost::assign::list_of("xsq-convert")("-i")(input_file.c_str())("-o")(output_dir.c_str())( "--extract-only"));
		
	// Assign libraries to each xsq-convert process
	unsigned nb_libs_per_process = libraries.size() / nb_processes;
	std::vector<std::string>::const_iterator lib = libraries.begin();
		
	BOOST_FOREACH(CommandLine& command_line, command_lines)
	{
	...
	CommandLines::iterator command_line = command_lines.begin();
...
{
	BOOST_FOREACH(const CommandLine& command_line, command_lines)
	{
		switch(fork())
		{
		case -1:
			...
			exit(-1);
		case 0:
			execvp("xsq-convert", (char* const *)(command_line.data()));
		default:
			break;
		}
	}
